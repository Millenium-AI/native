...
export const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 transition-transform duration-300 ease-in-out ${navHidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <div className="bg-native-green-secondary text-white py-2 px-4 text-xs sm:text-sm">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-center items-center gap-x-2 sm:gap-x-4 gap-y-1">
          ...
          <button onClick={() => handleNavClick('contact')} className="flex items-center gap-1 hover:underline whitespace-nowrap font-medium">
            Get a Quote
          </button>
          <span className="hidden sm:inline opacity-50">|</span>
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden text-native-gray">
